...
FLUENT_TOOLS = [
    Tool(name="fluent_launch", description="启动 Fluent solver 或连接现有实例",
         inputSchema={"type": "object", "properties": {
             "processors": {"type": "integer", "default": 4, "description": "启动新实例时的处理器数量"},
             "cwd": {"type": "string", "description": "启动新实例时的工作目录"},
             "port": {"type": "integer", "description": "连接现有 Fluent 实例的端口号，不填则启动新实例"},
             "ip": {"type": "string", "default": "127.0.0.1", "description": "连接现有实例的 IP 地址"},
             "password": {"type": "string", "description": "gRPC连接密码（连接远程/非localhost时提供）"},
             "connect_timeout": {"type": "integer", "default": 30, "description": "连接超时时间（秒），默认30秒，超时自动中断"}}}),
    Tool(name="fluent_read_case", description="加载 .cas 算例文件",
         inputSchema={"type": "object", "properties": {"file_path": {"type": "string"}}, "required": ["file_path"]}),
    Tool(name="fluent_read_mesh", description="加载 .msh 网格文件",
         inputSchema={"type": "object", "properties": {"file_path": {"type": "string"}}, "required": ["file_path"]}),
    Tool(name="fluent_set_solver", description="设置求解器（湍流模型/能量/瞬态）",
         inputSchema={"type": "object", "properties": {
             "viscous_model": {"type": "string", "enum": ["laminar", "k-epsilon", "k-omega", "sst", "spalart-allmaras"]},
             "energy": {"type": "boolean"}, "transient": {"type": "boolean"}}}),
    Tool(name="fluent_set_boundary", description="设置边界条件",
         inputSchema={"type": "object", "properties": {
             "zone": {"type": "string"}, "bc_type": {"type": "string"}, "params": {"type": "object"}},
             "required": ["zone", "bc_type"]}),
    Tool(name="fluent_set_material", description="设置区域材料",
         inputSchema={"type": "object", "properties": {"zone": {"type": "string"}, "material": {"type": "string"}},
             "required": ["zone", "material"]}),
    Tool(name="fluent_initialize", description="初始化流场（hybrid/standard）",
         inputSchema={"type": "object", "properties": {"method": {"type": "string", "enum": ["hybrid", "standard"], "default": "hybrid"}}}),
    Tool(name="fluent_iterate", description="迭代计算",
         inputSchema={"type": "object", "properties": {"iterations": {"type": "integer", "default": 100}}, "required": ["iterations"]}),
    Tool(name="fluent_get_residuals", description="获取残差值", inputSchema={"type": "object", "properties": {}}),
    Tool(name="fluent_save", description="保存 case/data",
         inputSchema={"type": "object", "properties": {"prefix": {"type": "string"}}, "required": ["prefix"]}),
    Tool(name="fluent_tui", description="执行 Fluent TUI 命令",
         inputSchema={"type": "object", "properties": {"command": {"type": "string"}}, "required": ["command"]}),
    Tool(name="fluent_status", description="Fluent 连接状态", inputSchema={"type": "object", "properties": {}}),
    Tool(name="fluent_load_udf", description="加载 UDF（自动处理 Windows 盘符路径问题，解释并验证成功）",
         inputSchema={"type": "object", "properties": {
             "source_file": {"type": "string", "description": "UDF 源文件绝对路径（如 E:/path/to/file.c）"},
             "compile": {"type": "boolean", "default": False, "description": "是否编译（默认 False 即解释执行）"}},
             "required": ["source_file"]}),
    Tool(name="fluent_hook_udf", description="将已加载的 UDF profile 挂钩到边界条件",
         inputSchema={"type": "object", "properties": {
             "zone_name": {"type": "string", "description": "边界条件区域名称"},
             "phase_name": {"type": "string", "default": "water", "description": "多相流相名称"},
             "profile_name": {"type": "string", "description": "UDF profile 函数名"},
             "momentum_field": {"type": "string", "default": "mass_flux", "description": "动量场类型（mass_flux/mass_flow_rate）"}},
             "required": ["zone_name", "profile_name"]}),
    Tool(name="fluent_list_udfs", description="列出已加载的 UDF 和边界条件挂钩状态",
         inputSchema={"type": "object", "properties": {
             "zone_name": {"type": "string", "description": "检查指定区域的 UDF 挂钩状态（可选）"}}}),
    Tool(name="fluent_exit", description="关闭 Fluent", inputSchema={"type": "object", "properties": {}}),
    Tool(name="fluent_get_script", description="获取当前 MCP 操作序列对应的 .jou 脚本（Skill 联动：自动累积 MCP→TUI 映射）",
         inputSchema={"type": "object", "properties": {}}),
    Tool(name="fluent_get_mapping_report", description="获取 MCP 到 TUI 的完整映射报告",
         inputSchema={"type": "object", "properties": {}}),
    Tool(name="fluent_reset_mapper", description="重置 TUI 映射器（清除历史记录）",
         inputSchema={"type": "object", "properties": {}}),
]

MECHANICAL_TOOLS = [
    Tool(name="mechanical_launch", description="连接/启动 Ansys Mechanical（指定端口则连接现有实例，否则启动 batch 模式）",
         inputSchema={"type": "object", "properties": {
             "port": {"type": "integer", "description": "连接现有实例的端口号，不填则启动新 batch 实例"}
         }}),
    Tool(name="mechanical_import", description="导入几何文件",
         inputSchema={"type": "object", "properties": {"file_path": {"type": "string"}}, "required": ["file_path"]}),
    ...
    Tool(name="mechanical_solve", description="执行求解", inputSchema={"type": "object", "properties": {}}),
    Tool(name="mechanical_get_result", description="提取结果",
         inputSchema={"type": "object", "properties": {
             "result_type": {"type": "string", "enum": ["total_deformation", "equivalent_stress", "equivalent_strain"]}},
             "required": ["result_type"]}),
    Tool(name="mechanical_list", description="列出几何体/Named Selections",
         inputSchema={"type": "object", "properties": {
             "what": {"type": "string", "enum": ["bodies", "named_selections"], "default": "bodies"}}}),
    Tool(name="mechanical_script", description="执行 IronPython 脚本",
         inputSchema={"type": "object", "properties": {"script": {"type": "string"}}, "required": ["script"]}),
    Tool(name="mechanical_status", description="Mechanical 连接状态", inputSchema={"type": "object", "properties": {}}),
    Tool(name="mechanical_exit", description="关闭 Mechanical", inputSchema={"type": "object", "properties": {}}),
]

GEOMETRY_TOOLS = [
    Tool(name="geometry_launch", description="启动 Geometry 建模器（Discovery/SpaceClaim 后端）",
         inputSchema={"type": "object", "properties": {}}),
    Tool(name="geometry_create_design", description="创建新的几何设计",
         inputSchema={"type": "object", "properties": {"name": {"type": "string", "default": "Design"}},
             "required": ["name"]}),
    Tool(name="geometry_create_block", description="创建立方体（v242: 用 sketch + extrude 实现）",
         inputSchema={"type": "object", "properties": {
    ...
    Tool(name="geometry_create_cylinder", description="创建圆柱体（extrude_sketch 方式）",
         inputSchema={"type": "object", "properties": {
             "name": {"type": "string", "default": "Cylinder"},
             "radius": {"type": "number", "default": 0.005}, "height": {"type": "number", "default": 0.01},
             "center_x": {"type": "number", "default": 0}, "center_y": {"type": "number", "default": 0},
             "center_z": {"type": "number", "default": 0}}, "required": ["name"]}),
    Tool(name="geometry_create_sphere", description="创建球体",
         inputSchema={"type": "object", "properties": {
             "name": {"type": "string", "default": "Sphere"}, "radius": {"type": "number", "default": 0.005},
             "center_x": {"type": "number", "default": 0}, "center_y": {"type": "number", "default": 0},
             "center_z": {"type": "number", "default": 0}}, "required": ["name"]}),
    Tool(name="geometry_export", description="导出几何为 STEP/IGES 格式",
         inputSchema={"type": "object", "properties": {
             "file_path": {"type": "string"},
             "format": {"type": "string", "enum": ["step", "iges"], "default": "step"}},
             "required": ["file_path"]}),
    Tool(name="geometry_list_bodies", description="列出当前设计中的所有几何体", inputSchema={"type": "object", "properties": {}}),
    Tool(name="geometry_import_file", description="导入 CAD 文件",
         inputSchema={"type": "object", "properties": {"file_path": {"type": "string"}}, "required": ["file_path"]}),
    Tool(name="geometry_status", description="Geometry 建模器连接状态", inputSchema={"type": "object", "properties": {}}),
    Tool(name="geometry_close", description="关闭 Geometry 建模器", inputSchema={"type": "object", "properties": {}}),
]
...
def _geom_get_design():
    """返回当前设计对象。依据官方文档：create_design 返回 Design，直接持有。"""
    global _current_design
    if _modeler is None:
        raise RuntimeError("Geometry 未连接，请先执行 geometry_launch")
    if _current_design is None:
        raise RuntimeError("无活跃设计，请先执行 geometry_create_design")
...
def _geom_create_cylinder(name: str, radius: float, height: float,
    ...
    return f"圆柱体 '{name}' 已创建: r={radius}m, h={height}m @ ({cx}, {cy}, {cz})"
...
def _geom_create_block(name: str, length: float, width: float, height: float,
    ...
    body = d.extrude_sketch(name=name, sketch=sketch, distance=height)
    return f"方块 '{name}' 已创建: {length}x{width}x{height}m @ ({cx}, {cy}, {cz})"


def _geom_create_sphere(name: str, radius: float, cx: float = 0, cy: float = 0, cz: float = 0) -> str:
    """v242: create_sphere 需要 v25.1+, 用 revolve_sketch 半圆旋转实现"""
    from ansys.geometry.core.sketch import Sketch
    from ansys.geometry.core.math import Point2D, Plane, Point3D, Vector3D
    ...
    return f"球体 '{name}' 已创建: r={radius}m @ ({cx}, {cy}, {cz})"
        ...
        if name == "fluent_launch":
            ...
            if port:
                ...
                try:
                    ...
                    raise TimeoutError(
                        f"连接 Fluent ({arguments.get('ip', '127.0.0.1')}:{port}) 超时 "
                        f"({connect_timeout}s)，请检查 Fluent 实例是否在运行、"
                        f"网络是否可达、防火墙是否放行端口 {port}"
                    )
                ver = _fluent_session.get_fluent_version()
                result = f"已连接 Fluent (端口 {port}, 版本: {ver})"
            else:
                ...
                result = f"Fluent 已启动 (版本: {_fluent_session.get_fluent_version()})"

        elif name.startswith("fluent_"):
            # --- MCP-TUI 映射器查询工具（无需 Fluent 会话） ---
            if name == "fluent_get_script":
                if get_mapping_report:
                    result = get_mapping_report()
                else:
                    result = "⚠ MCP-TUI 映射器未加载（Skill 脚本目录不存在）"
                ...
                if name == "fluent_read_case":
                    ...
                    result = f"已加载网格: {arguments['file_path']}"
                    ...
                    if "transient" in arguments:
                        s.setup.general.time = "transient" if arguments["transient"] else "steady"; changes.append(f"求解={'瞬态' if arguments['transient'] else '稳态'}")
                    ...
                    result = f"边界 '{arguments['zone']}' → {arguments['bc_type']}"
                    ...
                    result = f"区域 '{arguments['zone']}' 材料 → {arguments['material']}"
                    ...
                    if not matched:
                        ...
                        result = f"UDF '{profile}' 已挂钩到 {zone} / {pkey} / {field}"
                    ...
                    if zone_arg:
                        ...
                        result = "\n".join(lines) if len(lines) > 1 else f"区域 '{zone_arg}' 无 UDF 挂钩"
                    else:
                        lines = ["已加载区域及其挂钩状态:"]
                        for zone_name in s.setup.boundary_conditions:
                            try:
                                ...
                                pass
                        result = "\n".join(lines) if len(lines) > 1 else "无 UDF 挂钩"

                elif name == "fluent_status":
                    result = f"Fluent 已连接 | 版本: {s.get_fluent_version()}"
                elif name == "fluent_exit":
                    s.exit(); _fluent_session = None; result = "Fluent 已关闭"

                # --- MCP-TUI 自动映射（Skill 联动） ---
                if map_mcp_call:
        ...
        # ==================== MECHANICAL ====================
            ...
            if _mechanical_session is None:
                ...
                if name == "mechanical_import":
                    ...
                    result = f"已导入: {arguments['file_path']}"
                    ...
                    if raw and "|" in raw:
                        parts = raw.split("|")
                        result = (
                            f"Mechanical 已连接 | 几何体: {parts[0]} 个 | "
                            f"{parts[1]} | {parts[2]} nodes, {parts[3]} elements"
                        )
                    else:
                        result = f"Mechanical 已连接 | {raw}"
                elif name == "mechanical_exit":
                    m.exit(force=True); _mechanical_session = None; result = "Mechanical 已关闭"

        # ==================== GEOMETRY ====================
        elif name == "geometry_launch":
            from ansys.geometry.core import launch_modeler
            _modeler = launch_modeler(mode="discovery", version=242, timeout=180)
            result = "Geometry 建模器已启动 (Discovery v242)"

        elif name.startswith("geometry_"):
            if _modeler is None:
                result = "Geometry 未连接，请先执行 geometry_launch"
            else:
                if name == "geometry_create_design":
                    _current_design = _modeler.create_design(arguments["name"])
                    result = f"设计 '{_current_design.name}' 已创建"
                    ...
                    d = _geom_get_design()
                    # export_to_step/export_to_iges 将参数当作目录，在内部以设计名命名文件
                    # 直接传目录路径，文件名自动为 <design_name>.stp
                    ...
                    result = "Geometry 建模器已连接 (Discovery v242)"
                elif name == "geometry_close":
                    _modeler.close()
                    _modeler = None
                    result = "Geometry 建模器已关闭"
